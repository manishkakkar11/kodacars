...
import { ArrowLeft, Bus, Check, Download, ArrowRight, Loader2, AlertCircle, Shield } from 'lucide-react';
...
const ShuttleServicesPage: React.FC = () => {
  ...
  const whitepaperInfo = getWhitepaperInfo('shuttle-services');
  ...
  const handleDownload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const emailSent = await sendWhitepaperRequest({
        ...
        whitepaperType: 'Shuttle Services'
      });
  ...
  const handlePDFDownload = () => {
    if (whitepaperInfo) {
      downloadWhitepaper('shuttle-services');
    }
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-24">
          ...
          <div className="lg:col-span-7">
            ...
            <h1 className="text-4xl lg:text-6xl font-normal text-koda-black leading-[1.05] mb-10 tracking-tight">
              <span className="font-bold">Shuttle Services:</span> Reduce Shuttle Services Friction; Drive Superior Guest Satisfaction and Loyalty
            </h1>

            <div className="space-y-8 text-gray-700 font-medium leading-relaxed">
              <p className="text-xl lg:text-2xl text-koda-black">
                The transition from the hotel off-airport parking property—often referred to as the "last mile"—is a critical component of the guest experience. Static, unreliable shuttle services are a leading cause of negative initial impressions and declining customer satisfaction scores.
              </p>
              
              <p className="text-xl lg:text-2xl">
                The KodaCars AI Platform replaces fixed schedules and manual dispatching with a dynamic, real-time, personalized, and transparent transportation experience. This transforms the shuttle from a source of friction into a core service differentiator.
              </p>
              ...
              <div className="mt-16">
                ...
                <div className="space-y-10">
                  {[
                    "Data showing that properties with highly rated transportation services see up to a 30 % higher return rate for guests.",
                    "How Predictive Demand Forecasting uses flight arrival data to anticipate passenger volume spikes, leading to a 50 % Reduction in Average Guest Wait Time.",
                    "Insights into how Dynamic Routing constantly re-optimizes routes based on real-time vehicle GPS and traffic, which can reduce annual fuel consumption.",
                    "Strategies to use the Loyalty-Revenue Loop by leveraging the platform's integration with CRM to instantly recognize loyalty members and send proactive, targeted mobile offers.",
                    "Guidance on custom rewards modules that incentivize desired behaviors, such as awarding bonus loyalty points for the 5th ride or offering double points for off-peak travel booking."
                  ].map((point, idx) => (
            ...
            <div className="sticky top-32">
              {!submitted ? (
                ...
                <div className="bg-white border border-gray-200 p-12 text-center shadow-xl animate-fade-in-up">
                  ...
                  <p className="text-gray-600 text-lg mb-10 leading-relaxed">
                    Thank you, {formData.firstName}! Your guide to reducing shuttle service friction is now available for download.
                  </p>
                  <button 
                  ...
                  >
                    <Download size={24} />
                    Download PDF ({whitepaperInfo?.size || '3.2 MB'})
                  </button>
              ...
              <div className="mt-12 p-8 border border-gray-100 bg-gray-50/40 flex items-center gap-6">
                <div className="w-16 h-16 rounded-full bg-gray-300 overflow-hidden shrink-0">
                  <img src="https://images.unsplash.com/photo-1544620347-c4fd4a3d5957?auto=format&fit=crop&q=80&w=200" alt="Executive" className="w-full h-full object-cover" />
                </div>
                <div>
                  <p className="text-gray-600 italic font-medium leading-relaxed text-sm">
                    "Transitioning to dynamic shuttle routing cut our fuel costs and wait times simultaneously."
                  </p>
                  <p className="text-xs font-bold text-koda-black mt-2 uppercase tracking-widest">— Logistics Director, WestCoast Terminals</p>
                </div>
              </div>
            </div>
          </div>
...
export default ShuttleServicesPage;